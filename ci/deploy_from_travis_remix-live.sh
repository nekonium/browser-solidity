...
echo "Built website from \`$SHA\`. See https://github.com/nekonium/browser-solidity/ for details." >> README.md
...
eval `ssh-agent -s`
ssh-add deploy_key
git push -f git@github.com:nekonium/browser-solidity.git gh-pages
ssh-add -D
